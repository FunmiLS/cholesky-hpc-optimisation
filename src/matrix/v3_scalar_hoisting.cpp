/**
 * @file v3_scalar_hoisting.cpp
 * @brief Benchmark runner for the scalar-hoisting Cholesky version.
 * @details Runs several matrix sizes and logs three timings per case to CSV,
 * so optimisation progress can be compared fairly.
 */
...
/**
 * @brief Runs the matrix-size benchmark for v3 and saves results to CSV.
 * @return 0 on success, 1 if file output or factorisation fails.
 */
int main() {
    const std::string output_dir = "output";
    const std::string output_file = output_dir + "/v3_scalar_hoisting_matrix_size_runs.csv";
    ...
    for (int n : sizes) {
        ...
        for (int run = 0; run < 3; ++run) {
            ...
            const double elapsed_time = cholesky_v3_scalar_hoisting_impl(matrix.data(), n);
            if (elapsed_time < 0.0) {
                std::cerr << "Error: cholesky_v3_scalar_hoisting_impl failed for n=" << n << "\n";
                return 1;
            }
            csv << "," << elapsed_time;
        }
