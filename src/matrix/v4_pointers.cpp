/**
 * @file v4_pointers.cpp
 * @brief Benchmark runner for the pointer-based Cholesky version.
 * @details Sweeps matrix sizes and records three timings per size into CSV
 * for straightforward comparison with earlier versions.
 */
...
/**
 * @brief Runs the matrix-size benchmark for v4 and saves results to CSV.
 * @return 0 on success, 1 if file output or factorisation fails.
 */
int main() {
    const std::string output_dir = "output";
    const std::string output_file = output_dir + "/v4_pointers_matrix_size_runs.csv";
    ...
    for (int n : sizes) {
        ...
        for (int run = 0; run < 3; ++run) {
            ...
            // pointer in v4: row pointers (row_p/row_i) reduce repeated index arithmetic
            const double elapsed_time = cholesky_v4_pointers_impl(matrix.data(), n);
            if (elapsed_time < 0.0) {
                std::cerr << "Error: cholesky_v4_pointers_impl failed for n=" << n << "\n";
                return 1;
            }
            csv << "," << elapsed_time;
        }
