/**
 * @file v1_baseline.cpp
 * @brief Benchmark runner for the baseline Cholesky implementation.
 * @details Sweeps a list of matrix sizes and records three timings per size
 * into a CSV file for later comparison.
 */
...
/**
 * @brief Runs the matrix-size benchmark for v1 and saves results to CSV.
 * @return `0` on success, `1` if file output or factorisation fails.
 */
int main() {
    const std::string output_dir = "output";
    const std::string output_file = output_dir + "/v1_baseline_matrix_size_runs.csv";
    ...
    for (int n : sizes) {
        ...
        for (int run = 0; run < 3; ++run) {
            ...
            const double elapsed_time = cholesky_v1_baseline_impl(matrix.data(), n);
            if (elapsed_time < 0.0) {
                std::cerr << "Error: cholesky_v1_baseline_impl failed for n=" << n << "\n";
                return 1;
            }
            csv << "," << elapsed_time;
        }
