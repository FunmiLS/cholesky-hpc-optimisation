/**
 * @file v10_parallel_dynamic_threads.cpp
 * @brief Thread-scaling benchmark for dynamic-scheduled parallel Cholesky.
 * @details Performs a thread sweep on a fixed size, writing three
 * timings per thread count to CSV.
 */
...
/**
 * @brief Runs the thread-sweep benchmark for v10 and saves results to CSV.
 * @return 0 on success, 1 if file output or factorisation fails.
 */
int main() {
    const std::string output_dir = "output";
    const std::string output_file = output_dir + "/v10_parallel_dynamic_threads_runs.csv";
    // fix matrx size and block size
    const int n = 4096;
    ...
    for (int NUM = 1; NUM <= 70; ++NUM) {
        ...
        for (int run = 0; run < 3; ++run) {
            ...
            // openmp sweep runtime thread counts
            omp_set_num_threads(NUM);
            const double elapsed_time = cholesky_parallel_dynamic(matrix.data(), n, block_size);
            if (elapsed_time < 0.0) {
                std::cerr << "Error: cholesky_parallel_dynamic failed for n=" << n
