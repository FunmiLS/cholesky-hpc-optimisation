/**
 * @file v9_parallel_static_threads.cpp
 * @brief Thread-scaling benchmark for static-scheduled parallel Cholesky.
 * @details Performs a thread sweep with fixed matrix and block sizes, storing
 * three timings for each thread count in CSV.
 */
...
/**
 * @brief Runs the thread-sweep benchmark for v9 and saves results to CSV.
 * @return 0 on success, 1 if file output or factorisation fails.
 */
int main() {
    const std::string output_dir = "output";
    const std::string output_file = output_dir + "/v9_parallel_static_threads_runs.csv";
    // fixed matrix size and block size
    const int n = 4096;
