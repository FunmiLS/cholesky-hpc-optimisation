 ...
 * @brief Block-size benchmark for the cache-blocked Cholesky variant.
 * @details Uses a fixed matrix size and sweeps several block sizes, recording
 * three timings per block size in CSV output.
 */
...
/**
 * @brief Runs the block-size sweep for v6 and saves results to CSV.
 * @return 0 on success 1 if file output or factorisation fails.
 */
int main() {
    const std::string output_dir = "output";
    const std::string output_file = output_dir + "/v6_cache_blocking_block_size_runs.csv";
    const int n = 1024;
    ...
    csv << "matrix_size,block_size,run_1,run_2,run_3\n";
    ...
    for (int block_size : block_sizes) {
        ...
        for (int run = 0; run < 3; ++run) {
            ...
            if (elapsed_time < 0.0) {
                std::cerr << "Error: cholesky_v6_cache_blocking_impl failed for n=" << n
    ...
    std::cout << "Saved results to " << output_file
              << " using matrix_size=" << n
              << " and block_sizes={8,16,32,64,128,256,512}\n";
    return 0;
}
