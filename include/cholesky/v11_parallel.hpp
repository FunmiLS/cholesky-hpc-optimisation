#ifndef MPHIL_DIS_CHOLESKY_V11_PARALLEL_HPP
#define MPHIL_DIS_CHOLESKY_V11_PARALLEL_HPP
...
inline double cholesky_parallel(double *c, int n, int block_size_hint) {
    ...
    for (int k = 0; k < n; k += B) {
        ...
        // blocked panel solve for rows beneath the current diagonal block
        // openmp schedule(static) across independent block rows in panel solve
        // testing static here as the block has a lower overhead 
        #pragma omp parallel for num_threads(NUM) schedule(static)
        ...
        // openmp collapse(2) + schedule(dynamic) across (i,j) block pairs
        // using dynamic and collapse for better load balance as block sizes are uneven
        #pragma omp parallel for num_threads(NUM) collapse(2) schedule(dynamic)
        for (int i = k + block_size; i < n; i += B) {
