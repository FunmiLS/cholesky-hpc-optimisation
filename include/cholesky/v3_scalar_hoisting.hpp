#ifndef MPHIL_DIS_CHOLESKY_V3_SCALAR_HOISTING_HPP
#define MPHIL_DIS_CHOLESKY_V3_SCALAR_HOISTING_HPP
...
inline double cholesky_v3_scalar_hoisting_impl(double *c, int n) {
    if (n <= 0 || n > 100000) {
    ...
    for (int p = 0; p < n; ++p) { // move along the diagonal of the matrix
        ...
        for (int j = p + 1; j < n; ++j) { // update submatrix below-right
            const double c_pj = c[n * p + j]; // scalar hoisting, load once per j reuse in inner i loop
            for (int i = p + 1; i < n; ++i) { // of diagonal element
                c[n * i + j] -= c[n * i + p] * c_pj;
            }
        }
    }
