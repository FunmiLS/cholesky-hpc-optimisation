#ifndef MPHIL_DIS_CHOLESKY_V9_PARALLEL_STATIC_HPP
#define MPHIL_DIS_CHOLESKY_V9_PARALLEL_STATIC_HPP
...
inline double cholesky_parallel_static(double *c, int n, int block_size_hint) {
    ...
    for (int k = 0; k < n; k += B) {
        ...
        for (int p = k; p < k + block_size; ++p) { // move along the diagonal of the matrix
            ...
            // scalar hoisting reused in inner loops
        ...
        // openmp static schedule across independent block rows in panel solve
        // static to investigate predictable work distribution whilst keeping original parallelisation strategy
        ...
        // openmp static schedule across independent block rows
        // static to investigate predictable work distribution 
        #pragma omp parallel for num_threads(NUM) schedule(static)
        for (int i = k + block_size; i < n; i += B) {
            const int cur_B_i = std::min(B, n - i);
            for (int j = k + block_size; j < n; j += B) {
                ...
                for (int pi = i; pi < i + cur_B_i; ++pi) {
                    ...
                    for (int pk = k; pk < k + block_size; ++pk) {
                        ...
                        // pointer acess row_pk points to row pk
