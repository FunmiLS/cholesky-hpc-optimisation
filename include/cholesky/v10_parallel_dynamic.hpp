#ifndef MPHIL_DIS_CHOLESKY_V10_PARALLEL_DYNAMIC_HPP
#define MPHIL_DIS_CHOLESKY_V10_PARALLEL_DYNAMIC_HPP
...
inline double cholesky_parallel_dynamic(double *c, int n, int block_size_hint) {
    ...
    // cache blocking process matrix in BxB tiles
    for (int k = 0; k < n; k += B) {
        ...
        // openmp dynamic schedule across independent block rows in panel solve
        // investigation to see if dynamic scheduling improves load balance on vanilla implementation
        #pragma omp parallel for num_threads(NUM) schedule(dynamic)
        ...
        // openmp dynamic schedule across independent block rows in trailing update
        #pragma omp parallel for num_threads(NUM) schedule(dynamic)
        for (int i = k + block_size; i < n; i += B) {
            const int cur_B_i = std::min(B, n - i);
