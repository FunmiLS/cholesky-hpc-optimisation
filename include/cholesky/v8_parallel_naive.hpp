#ifndef MPHIL_DIS_CHOLESKY_V8_PARALLEL_NAIVE_HPP
#define MPHIL_DIS_CHOLESKY_V8_PARALLEL_NAIVE_HPP
...
inline double cholesky_parallel_naive(double *c, int n, int block_size_hint)
{
    ...
    // openmp runtime thread count used for parallel regions
    ...
    for (int k = 0; k < n; k += B)
    {
        ...
        { // move along the diagonal of the matrix
            ...
            for (int j = p + 1; j < k + block_size; ++j)
            { // update row to right of diagonal element
                row_p[j] *= inv_diag;
            }

            for (int i = p + 1; i < k + block_size; ++i)
...
// openmp parallelise independent block rows
...
// blocked submatrix update
// openmp parallelise independent block rows in trailing update
#pragma omp parallel for num_threads(NUM)
        for (int i = k + block_size; i < n; i += B)
        {
            const int cur_B_i = std::min(B, n - i);
